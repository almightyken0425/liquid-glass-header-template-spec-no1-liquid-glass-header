# CLAUDE.md

本 repo 為 LiquidGlassHeaderTemplate 產品的 **Module Spec git**，module_id 為 `no1_liquid_glass_header`。目前為空殼，尚無 Spec 內容。

## 多層 git 配對

- **頂層 Product git：** `../../`
- **對側 Module Design git：** 尚無，模板庫視覺由 Impl 直接承載
- **本 Module Spec git：** 即本 repo（位於 `no3_product_specs/no1_liquid_glass_header/`）
- **對側 Module Impl git：** `no5_product_development/no1_liquid_glass_header/`

## 當前狀態

本 Spec git 為空殼，對應的 Impl git 為模板庫，提供 React Native Liquid Glass 風格 Header 的實作。當需要記錄模板 API、使用說明或 component 行為規格時，在此補 `no1_data_models/`、`no2_screens/`、`no3_logics/` 三層的檔案。

---

## 撰寫規範

所有規格文件依循 `spec_writer` 分層政策與 `universal_writing_linter` 通用政策。
